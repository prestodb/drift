        ...
        extends AbstractProcessor
{
    ...
    private void extract(TypeElement typeElement)
    {
        ...
        TypeSpec.Builder typeSpec = TypeSpec.classBuilder(className + ThriftAnnotations.META_SUFFIX)
        ...
        for (Element element : elements().getAllMembers(typeElement)) {
            ...
            if ((element instanceof ExecutableElement) &&
                    (isAnnotatedWith(element, ThriftAnnotations.THRIFT_METHOD) || isAnnotatedWith(element, ThriftAnnotations.THRIFT_FIELD))) {
            ...
            else if (((element instanceof VariableElement) && isAnnotatedWith(element, ThriftAnnotations.THRIFT_FIELD)) ||
        ...
        String name = typeElement.getQualifiedName() + ThriftAnnotations.META_SUFFIX;

        try (Writer writer = filer().createSourceFile(name, typeElement).openWriter()) {
            javaFile.writeTo(writer);
        }
        catch (IOException e) {
            log(ERROR, String.format("Failed to create %s%s file", typeElement, ThriftAnnotations.META_SUFFIX));
        }
    }
