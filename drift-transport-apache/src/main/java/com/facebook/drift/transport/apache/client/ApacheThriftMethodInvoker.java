        ...
        implements MethodInvoker
{
            ...
            throws TException, org.apache.thrift.TException
    {
        ...
        if (message.type != REPLY) {
            throw new TApplicationException(TApplicationException.Type.INVALID_MESSAGE_TYPE, format("Received invalid message type %s from server", message.type));
        }
        if (!message.name.equals(method.getName())) {
            throw new TApplicationException(TApplicationException.Type.WRONG_METHOD_NAME, format("Wrong method name in reply: expected %s but received %s", method.getName(), message.name));
        }
