                ...
                throws Exception
        {
            ...
            try {
                ...
                if (message.getType() != REPLY) {
                    throw new TApplicationException(TApplicationException.Type.INVALID_MESSAGE_TYPE, format("Received invalid message type %s from server", message.getType()));
                }
                if (!message.getName().equals(method.getName())) {
                    throw new TApplicationException(TApplicationException.Type.WRONG_METHOD_NAME, format("Wrong method name in reply: expected %s but received %s", method.getName(), message.getName()));
                }
                if (message.getSequenceId() != sequenceId) {
                    throw new TApplicationException(TApplicationException.Type.BAD_SEQUENCE_ID, format("%s failed: out of sequence response", method.getName()));
                }
                ...
                if (results == null) {
                    throw new TApplicationException(TApplicationException.Type.MISSING_RESULT, format("%s failed: unknown result", method.getName()));
                }
                return results;
            }
            finally {
