    ...
    protected final void decode(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out)
    {
        ...
        try {
            ...
            if (frameLength > maxFrameSize) {
                FrameInfo frameInfo = new FrameInfo(message.getName(), message.getType(), message.getSequenceId(), Transport.UNFRAMED, protocol, assumeClientsSupportOutOfOrderResponses);
